...
@Service
public class RestaurantLogoService {
    ...
    public RestaurantLogoService(@Value("${app.upload-dir:uploads}") String uploadDir) {
        this.uploadRoot = Paths.get(uploadDir).toAbsolutePath().normalize().resolve("restaurants");

        try {
            Files.createDirectories(uploadRoot);
        } catch (IOException e) {
            throw new IllegalStateException("Unable to initialize restaurant logo upload directory", e);
        }
    }
    ...
    public String store(Long restaurantId, MultipartFile file) {
        ...
        String storedFilename = "logo-" + System.currentTimeMillis() + extension;
        ...
        try {
            ...
            throw new IllegalStateException("Unable to store restaurant logo", e);
        }
    ...
    public Resource loadAsResource(Long restaurantId, String filename) {
        try {
            ...
            if (!resource.exists() || !resource.isReadable()) {
                throw new ResourceNotFoundException("Restaurant logo not found");
            }
    ...
    private void validate(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new BadRequestException("Restaurant logo file is required");
        }

        if (file.getSize() > MAX_FILE_SIZE_BYTES) {
            throw new BadRequestException("Restaurant logo must be 5MB or smaller");
        }
